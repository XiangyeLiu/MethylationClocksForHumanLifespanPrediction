...
- For a small seet of data, only a couple of seconds may be need to run codes on a personal computer except resampling procedures, which may take over a dozen minutes. Note that the run time is highly correlated with data size, thus the expected run time is not guaranteed.
